...
Проведем первоначальную настройку маршрутизатора provider-sw-1 (рис. [-@fig:001]). Зададим имя, настроим доступ по паролю.

![Первоначальная настройка маршрутизатора provider-sw-1](image/1.png){#fig:001 width=70%}

Проведем первоначальную настройку коммутатора provider-gw-1 (рис. [-@fig:002]). Зададим имя, настроим доступ по паролю.

![Первоначальная настройка коммутатора provider-gw-1](image/2.png){#fig:002 width=70%}
...
![Настройка интерфейсов коммутатора provider-sw-1](image/5.png){#fig:004 width=70%}

Проведем настройку интерфейсов маршрутизатора msk-donskaya-gw-1. Поднимем интерфейс f0/1, создадим интерфейс f0/1.4 для 4 vlan и зададим ip-адрес. (рис. [-@fig:005])

![Настройка интерфейсов маршрутизатора msk-donskaya-gw-1](image/8.png){#fig:005 width=70%}

Пропингую адрес 192.0.2.1. Пинг проходит успешно (рис. [-@fig:006])

![Проверка доступности маршрутизатора](image/4.png){#fig:006 width=70%}

Назначу роутер по умолчанию для gw-1. (рис. [-@fig:007])

![Настройка роутера по умолчанию](image/8.png){#fig:007 width=70%}

Попытка пропинговать устройство 198.51.100.1 оказывается неудачной и для этого я прервала связь, и тогда все заработало.

![Попытка пропинговать 198.51.100.1](image/11.png){#fig:008 width=70%}

Настроим пул адресов.

![Настройка пула адресов](image/12.png){#fig:897 width=70%}

Совершим настройку PAT. (рис. [-@fig:009])

![Настройка PAT](image/12.1.png){#fig:009 width=70%}

Настроим Port Address Translation (PAT) на суб-интерфейсах маршрутизатора с территории Донская (рис. [-@fig:010]).

![Настройка PAT](image/13.1.png){#fig:010 width=70%}

Добавила ноутбук laptop-ext в схему в область internet.
Проверка доступности к маршрутизаторам от ноутбука admin. Все доступны.

![Пинг](image/13.png){#fig:011 width=70%}

![Пинг](image/14.png){#fig:012 width=70%}

![Пинг](image/15.png){#fig:013 width=70%}

Проверила возможность выхода в интернет после всех настроек

![www.yandex.ru](image/17.png){#fig:014 width=70%}

Страница отобразилась. Также проверила через браузер на устройстве laptop-ext доступ к адресу 198.51.100.2, он оказался доступен.
...
Он заключается в том, чтобы осуществлять перевод частного локального IP-адреса в общедоступный глобальный IP-адрес и наоборот. Это необходимо для обеспечения доступа к Интернету локальным узлам, использующим частные адреса.
