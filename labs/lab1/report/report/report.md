...
![Конфигурация маршрутизатора](image/12.png){#fig:012 width=70%}
...
Сначала посылаются пакеты ARP, затем ICMP. В сети с хабом рассылка идет по всем устройствам, а в сети с коммутатором только к пункту назначения. После получения пакета идет идет рассылка STP пакетов всем устройствам сети. Пакеты CDP(англ. Cisco Discovery Protocol) — проприетарный протокол второго уровня, разработанный компанией Cisco Systems не появились.

![Нет CDP пакетов](image/14.png){#fig:014 width=70%}
Теоретически исследуем структуру пакета CDP. 
